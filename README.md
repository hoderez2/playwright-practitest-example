...
One invocation does a single find → claim → run → report → complete cycle: it finds a queued Test Set (via the saved filter, or `--set-id`), flips its status to `Claimed`, resolves each instance's Test to an Automation ID, flips status to `Running`, runs only the matching Playwright tests, reports each result into its existing instance via `runs.json` + `instance-id` (as opposed to `auto_create`), and flips status to `Completed`/`Failed`. This is a one-shot script rather than a background poller, by design - reliable to trigger and narrate live; a continuous loop is a natural next step (wrap the same logic in an interval).

**`Completed` vs `Failed` tracks the orchestration, not the tests.** A run where some (or all) tests fail is still `Completed`, as long as it ran to completion and reported every result - individual pass/fail is already visible per-instance in PractiTest, so the Test Set's status doesn't re-encode it. `Failed` is reserved for the run itself breaking: couldn't fetch instances, no tests had a matching Automation ID, Playwright failed to launch, or any other exception after claiming - which also now reliably marks the set `Failed` instead of leaving it stuck in `Claimed`/`Running`.
